...
use Crustum\Mcp\Test\Fixtures\CacheableResource;
use Crustum\Mcp\Test\Fixtures\ExampleServer;

/**
 * Dispatch a JSON-RPC message through a fresh server.
 *
 * @param array<string, mixed> $message JSON-RPC message
 * @param \Closure|null $factory Server factory
 * @return array<string, mixed>
 */
function cacheTestResultFor(array $message, ?Closure $factory = null): array
{
...
it('leaves caching hints off a result that is not cacheable', function (): void {
    $result = cacheTestResultFor(callToolMessage())['result'];
...
it('uses the hint the server attribute declares', function (): void {
    $result = cacheTestResultFor(listToolsMessage(), fn(ArrayTransport $transport): Server => new #[Cacheable(ttlMs: 300000, scope: CacheScope::Public)] class ($transport) extends ExampleServer {
    })['result'];
...
it('prefers the per method hint over the server attribute', function (): void {
    ...
    expect(cacheTestResultFor(listToolsMessage(), $server)['result'])
        ->ttlMs->toBe(60000)
        ->cacheScope->toBe('private');

    $listPrompts = ['jsonrpc' => '2.0', 'id' => 1, 'method' => 'prompts/list', 'params' => ['_meta' => protocolMeta()]];

    expect(cacheTestResultFor($listPrompts, $server)['result'])
        ->ttlMs->toBe(300000)
...
it('prefers the resource attribute over the server hints', function (): void {
    ...
    expect(cacheTestResultFor(readResourceMessage('file://resources/cacheable-resource'), $server)['result'])
        ->ttlMs->toBe(120000)
        ->cacheScope->toBe('public');

    expect(cacheTestResultFor(readResourceMessage('file://resources/last-log-line-resource'), $server)['result'])
        ->ttlMs->toBe(60000)
        ->cacheScope->toBe('private');
});

it('never emits a negative ttl', function (): void {
    $result = cacheTestResultFor(listToolsMessage(), fn(ArrayTransport $transport): Server => new #[Cacheable(ttlMs: -1)] class ($transport) extends ExampleServer {
    })['result'];
...
it('returns tools in the same order on every request', function (): void {
    $names = fn(): array => array_column(cacheTestResultFor(listToolsMessage())['result']['tools'], 'name');
