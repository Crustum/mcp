...
it('uses the computed Claude domain when not set on app resource', function (): void {
    ...
    ServerUrl::resolveUsing(fn(): string => $currentUrl);

    try {
        $expectedDomain = Str::of(hash('sha256', $currentUrl))
            ->limit(32, '')
...
it('attribute domain overrides auto-resolved domain', function (): void {
    ...
    $resource = new #[AppMetaAttribute(domain: 'custom.example.com')]
    class extends AppResource
    {
        public function handle(Request $request): Response
        {
            return Response::text('<html></html>');
...
it('configures ui meta via attribute', function (): void {
    $resource = new #[AppMetaAttribute(connectDomains: ['https://api.example.com'], permissions: [Permission::Camera, Permission::ClipboardWrite], prefersBorder: true, domain: 'sandbox.example.com')]
    class extends AppResource
    {
        public function handle(Request $request): Response
        {
