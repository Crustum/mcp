...
use Crustum\Mcp\Test\Fixtures\AnnotatedResource;
use Crustum\Mcp\Test\Fixtures\DailyPlanResource;
...
it('reads html content from a relative file path', function (): void {
    $path = ROOT . DS . 'test-response.html';

    file_put_contents($path, '<html><body>From File</body></html>');
...
it('creates a response from a CakePHP view template', function (): void {
    $viewDir = APP . 'templates';

    if (!is_dir($viewDir)) {
        mkdir($viewDir, 0755, true);
    }

    file_put_contents($viewDir . DS . 'mcp_view_test.php', '<p><?= h($name) ?></p>');

    $response = Response::view('mcp_view_test', ['name' => 'Test']);

    expect($response->content())->toBeInstanceOf(Text::class)
        ->and((string)$response->content())->toContain('<p>Test</p>')
        ->and($response->isError())->toBeFalse();

    @unlink($viewDir . DS . 'mcp_view_test.php');
});
