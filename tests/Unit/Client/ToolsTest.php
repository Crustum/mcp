...
it('returns a collection of tools keyed by name', function (): void {
    ...
    expect($tools)
        ...
        ->toHaveCount(2)
        ->and(collectionKeys($tools))->toBe(['add', 'subtract'])
        ->and(collectionGet($tools, 'add'))
...
it('auto-paginates tools/list until nextCursor is absent', function (): void {
    ...
    expect(collectionKeys($tools))->toBe(['first', 'second', 'third'])
...
it('stops paginating once limit is reached without fetching the next page', function (): void {
    ...
    expect(collectionKeys($tools))->toBe(['a', 'b'])
...
it('calls a bound tool returned from tools()', function (): void {
    ...
    $tools = (new Client($transport))->tools();
    $tool = collectionGet($tools, 'say-hi');
