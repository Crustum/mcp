...
it('maps a connection failure to a ClientException', function (): void {
    HttpFake::throwNetworkException('timed out');
...
it('drives a full handshake and tools list over HTTP via Client::web', function (): void {
    ...
    expect(collectionKeys($tools))->toBe(['add']);
...
it('interoperates with a server negotiated down to 2025-06-18', function (): void {
    ...
    expect(collectionKeys($tools))->toBe(['add'])
        ->and(collectionGet($tools, 'add')->title)->toBe('Add Numbers')
        ->and(collectionGet($tools, 'add')->outputSchema)->toBe([
            'type' => 'object',
            'properties' => ['sum' => ['type' => 'number']],
        ])
