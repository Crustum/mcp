...
it('returns a collection of resources keyed by uri', function (): void {
    ...
    expect($resources)
        ...
        ->toHaveCount(2)
        ->and(collectionKeys($resources))->toBe(['file://readme', 'file://license'])
        ->and(collectionGet($resources, 'file://readme'))
        ...
        ->size->toBe(1024)
        ->and(collectionGet($resources, 'file://license'))
        ->title->toBeNull()
...
it('auto-paginates resources/list until nextCursor is absent', function (): void {
    ...
    expect(collectionKeys($resources))->toBe(['file://first', 'file://second', 'file://third'])
...
it('stops paginating once the resource limit is reached without fetching the next page', function (): void {
    ...
    expect(collectionKeys($resources))->toBe(['file://a', 'file://b'])
...
it('preserves title, annotations and _meta on a resource', function (): void {
    ...
    $resource = collectionGet((new Client($transport))->resources(), 'file://readme');
