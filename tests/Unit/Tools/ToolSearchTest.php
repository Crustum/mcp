...
use Crustum\Mcp\Server\Tools\ExecuteTools;
use Crustum\Mcp\Server\Tools\SearchTools;
...
use Crustum\Mcp\Test\Fixtures\StreamingTool;
use Crustum\Mcp\Test\Fixtures\StructuredContentTool;

/**
 * Build a server context with a ToolSearch catalog.
 *
 * @param array<int, \Crustum\Mcp\Server\Tool|string> $tools Catalog tools
...
/**
 * Find a tool by name in a server context.
 *
 * @param \Crustum\Mcp\Server\ServerContext $context Server context
 * @param string $name Tool name
 * @return \Crustum\Mcp\Server\Tool
 */
...
/**
 * Call a tool through the tools/call method and return the final result.
 *
 * @param \Crustum\Mcp\Server\ServerContext $context Server context
...
/**
 * Call a tool through the tools/call method and return every response frame.
 *
 * @param \Crustum\Mcp\Server\ServerContext $context Server context
 * @param \Crustum\Mcp\Server\Tool $tool Tool instance
 * @param array<string, mixed> $arguments Tool arguments
...
it('supports declaring a searchable tool group on the server', function (): void {
    ...
    $tools = $context->tools();
    $searchTools = $tools->filter(fn(Tool $tool): bool => $tool->name() === 'search_tools')->first();

    expect(array_map(fn(Tool $tool): string => $tool->name(), $tools->toList()))->toBe([
...
it('enforces the configured call and output limits', function (): void {
    Configure::write('Mcp.tool_search.max_tool_calls', 1);
    Configure::write('Mcp.tool_search.max_output_bytes', 256);
    ...
    try {
        ...
        Configure::delete('Mcp.tool_search.max_tool_calls');
        Configure::delete('Mcp.tool_search.max_output_bytes');
    }
});
